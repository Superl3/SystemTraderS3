...
class MetricsTests(unittest.TestCase):
    def test_metrics_json_created_for_exported_default_run(self) -> None:
        ...
        self.assertEqual("2.000000", payload["per_symbol"]["SIM"]["total_realized_pnl"])
        self.assertEqual(2, payload["per_symbol"]["SIM"]["total_number_of_trades"])
        self.assertEqual(2, payload["turnover"]["fill_count"])
        self.assertEqual("202.000000", payload["turnover"]["total_traded_notional"])
        self.assertEqual("0.002020", payload["turnover"]["turnover_ratio"])
    ...
    def test_metrics_calculates_drawdown_and_trade_outcomes(self) -> None:
        ...
        self.assertEqual("5.000000", payload["per_symbol"]["UNAVAILABLE"]["total_realized_pnl"])
        self.assertEqual(2, payload["per_symbol"]["UNAVAILABLE"]["total_number_of_trades"])
        self.assertNotEqual("UNAVAILABLE", payload["cagr_pct"])
        self.assertEqual("UNAVAILABLE", payload["benchmark_relative"]["alpha_pct"])
        self.assertEqual("UNAVAILABLE", payload["turnover"]["turnover_ratio"])
        self.assertIn("turnover unavailable because fills.csv is missing.", payload["gaps"])
    ...
    def test_benchmark_relative_metrics_calculate_from_equity_columns(self) -> None:
        ...
        self.assertEqual("2.000000", relative["upside_capture_ratio"])
        self.assertEqual("UNAVAILABLE", relative["downside_capture_ratio"])
    ...
    def test_benchmark_relative_beta_and_volatility_math(self) -> None:
        ...
        self.assertEqual("2.000000", relative["upside_capture_ratio"])
        self.assertEqual("2.000000", relative["downside_capture_ratio"])
    ...
    def test_metrics_work_on_multisymbol_portfolio_equity(self) -> None:
        ...
        self.assertEqual("0.000000", payload["max_drawdown_pct"])
        self.assertIn("AAA", payload["per_symbol"])
        self.assertIn("BBB", payload["per_symbol"])
        self.assertEqual("2.785119", payload["turnover"]["turnover_ratio"])
        self.assertEqual("1.874600", payload["turnover"]["buy_turnover_ratio"])
        self.assertEqual("0.910520", payload["turnover"]["sell_turnover_ratio"])
